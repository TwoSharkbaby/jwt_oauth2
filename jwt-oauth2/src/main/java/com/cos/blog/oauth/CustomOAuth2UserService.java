...
import com.cos.blog.model.CustomOAuth2User;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		System.out.println("CustomOAuth2UserService - loadUser 16");

		/**
		 * DefaultOAuth2UserService 객체를 생성하여, loadUser(userRequest)를 통해
		 * DefaultOAuth2User 객체를 생성 후 반환 DefaultOAuth2UserService의 loadUser()는 소셜 로그인
		 * API의 사용자 정보 제공 URI로 요청을 보내서 사용자 정보를 얻은 후, 이를 통해 DefaultOAuth2User 객체를 생성 후
		 * 반환한다. 결과적으로, OAuth2User는 OAuth 서비스에서 가져온 유저 정보를 담고 있는 유저
		 */
	...
	private SocialType getSocialType(String registrationId) {
		System.out.println("로그인한 쇼셜 = "+registrationId);
		System.out.println("CustomOAuth2UserService - getSocialType 17");
	...
	/**
	 * SocialType과 attributes에 들어있는 소셜 로그인의 식별값 id를 통해 회원을 찾아 반환하는 메소드 만약 찾은 회원이
	 * 있다면, 그대로 반환하고 없다면 saveUser()를 호출하여 회원을 저장한다.
	 */
	private User getUser(OAuthAttributes attributes, SocialType socialType) {
		System.out.println("CustomOAuth2UserService - getUser 20");
	...
	private User saveUser(OAuthAttributes attributes, SocialType socialType) {
		System.out.println("CustomOAuth2UserService - saveUser 22");
